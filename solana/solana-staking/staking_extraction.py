...
class SolanaAPI:
    ...
    def __init__(self):
        ...
        self.headers = {'Authorization': self.figment_api_key, 'Content-Type': 'application/json'}
    ...
    def get_current_epoch_info(self):
        """Extracts information about the current epoch."""
        info = self.api.get_epoch_info().get('result')
        inflation = self.api.get_inflation_rate().get('result')
        data = pd.merge(pd.DataFrame.from_dict(info, orient='index').T,
                        pd.DataFrame.from_dict(inflation, orient='index').T,
                        on='epoch')
        return data

    def get_token_list(self) -> pd.DataFrame:
        """Extracts the list of all tokens existing on Solana through solanascan API."""
    ...
    def get_transaction_information(self, transaction_hash: str):
        """Extracts detailed information about a specific transaction from Solana RPC API."""
        ...
        response = requests.post(url='https://api.mainnet-beta.solana.com',
    ...
    def get_all_transactions(self, start_date: datetime.datetime, address: str = None, limit=50):
        ...
        while True:
            response = requests.get(url,
                                    ...
                                    params=dict(limit=limit,
                                                account=address,
                                                beforeHash=before_hash)).json()
            txs_list.append(pd.DataFrame(response))

            if len(response) == 0:
                print(f"No transactions returned for address {address} "
    ...
    def get_solana_transfers(self, start_date: datetime.datetime, address: str = None, limit=100):
        """Extracts solana transfers from all transactions of a given account. Are filtered out the programs
        related transactions as well as the token transfers. """
        ...
        if len(sol_transfers) > 0:
            ...
            data = pd.concat([data.reset_index(drop=True).drop(['meta', 'transaction'], axis=1),
                              pd.json_normalize(data['meta']).reset_index(drop=True),
                              pd.json_normalize(data['transaction']).reset_index(drop=True)], axis=1)
    ...
    def get_cluster_nodes(self):
        """Extracts the list of all cluster nodes at the execution date."""
    ...
    def get_validators_snapshot(self):
        """Extracts the current and delinquent validators list."""
    ...
    def get_validators_vote_credits(self):
        """Extracts all the validators and their balance per epoch. NB: This method only work since epoch 275."""
    ...
    def get_delegators_snapshot(self, vote_key: str = None):
        """Extracts all the delegator of a specific validator program. The address to be used is one of the
        validators' votePubkey values."""
    ...
    def get_delegators_stakes(self, vote_key: str = None):
        """Extracts the full staking history of a validator node's exhaustive list of delegators from Solanascan."""
        ...
        for delegator_address in tqdm(delegators['staker']):
            response = requests.get(url,
                                    headers=headers,
                                    params=dict(account=delegator_address)).json()
            stake_accounts = pd.DataFrame(response.values())
            stake_activations = [self.api.get_stake_activation(stake_account).get('result')
                                 for stake_account in stake_accounts['stakeAccount']]
    ...
    def get_delegators_rewards(self, addresses: list = None, start_epoch: int = None):
        "Extracts the inflation reward for all the ledger delegators."
